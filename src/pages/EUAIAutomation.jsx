...
import {
  ...
  Award, Bot, Cpu, MapPin, Factory, AlertTriangle, FileText, Globe
} from 'lucide-react'
...
const euAIFeatures = [
  {
    ...
    description: 'AI-powered analysis of city emissions data across all sectors with trend detection and cross-country benchmarking.',
    icon: Factory,
    status: 'available',
    phase: 'Foundation',
  ...
  },
  {
    ...
    description: 'Machine learning models to predict climate hazard impacts based on historical Q2 data and regional climate models.',
    icon: AlertTriangle,
    status: 'available',
    phase: 'Foundation',
    currentState: 'Static hazard reporting',
    benefit: 'Predictive risk scoring across 39 countries'
  },
  {
    id: 'target_tracking',
  ...
  },
  {
    ...
    description: 'AI recommendations for climate actions based on successful implementations from peer cities across Europe.',
    icon: Zap,
    status: 'coming_soon',
    phase: 'Enhancement',
    currentState: 'No cross-city learning',
    benefit: 'Personalized action plans from 173 EU cities'
  },
  {
    id: 'report_generator',
  ...
  },
  {
    ...
    name: 'Cross-Country Benchmarking',
    description: 'AI-driven benchmarking against similar cities based on size, region, and climate profile across Europe.',
    icon: PieChart,
    status: 'coming_soon',
    phase: 'Optimization',
...
const expectedOutcomes = [
  ...
  { metric: '39 Countries', description: 'Cross-border climate insights', icon: 'TrendingUp' },
...
const implementationRoadmap = [
  {
    ...
    status: 'in_progress',
    actions: [
      'Multi-language data standardization',
      'Regional hazard classification AI',
      'Cross-border emissions validation'
    ]
  },
  {
    ...
    status: 'planned',
    actions: [
      'EU-wide target tracking',
      'Cross-country action learning',
      ...
      <div className="p-5">
        ...
        {feature.status === 'available' ? (
          <button
            ...
            className="w-full py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg font-medium hover:from-green-700 hover:to-emerald-700 transition-all flex items-center justify-center space-x-2"
          >
    ...
    switch (feature?.id) {
      ...
      case 'hazard_prediction':
        return [
          ...
          'Processing European climate models...',
          ...
          setTimeout(() => {
            ...
            setResults({
              ...
              insights: [
                ...
                `Processed data for ${city?.name || 'selected city'}`,
                'Regional benchmarks applied',
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">AI-powered analysis for European cities</p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div className="p-4 bg-green-50 rounded-lg">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-green-700">
                    {city?.name || 'All EU Cities'} {city?.country ? `(${city.country})` : ''}
                  </span>
                </div>
              ...
              <div className="flex items-center space-x-2 p-3 bg-emerald-50 rounded-lg">
                ...
                <span className="text-sm text-emerald-700">
                  AI will analyze CDP data with European regional context
                </span>
              </div>
            </div>
          ) : analyzing ? (
        ...
        <div className="p-6 border-t border-gray-100 flex space-x-3">
          ...
          {!analyzing && !results && (
            <button
              ...
              className="flex-1 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg font-medium hover:from-green-700 hover:to-emerald-700 transition-all flex items-center justify-center space-x-2"
            >
              <Sparkles className="w-4 h-4" />
...
function EUAIAutomation() {
  const { selectedId } = useOrganization()
  const [showModal, setShowModal] = useState(false)
  const [activeFeature, setActiveFeature] = useState(null)

  const selectedCity = euCitiesFull.find(c => c.id === selectedId)
  ...
  const stats = {
    totalCities: euCitiesFull.length,
    countries: [...new Set(euCitiesFull.map(c => c.country))].length,
    withEmissions: euCitiesFull.filter(c => c.q3_1_3?.length > 0).length,
    withHazards: euCitiesFull.filter(c => c.q2_2?.length > 0).length,
    withTargets: euCitiesFull.filter(c => c.q6_1_1?.length > 0).length
  }
    ...
    <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-green-100">
              Transform European city climate reporting with intelligent automation
            </p>
          </div>
          <div className="text-right">
            ...
            <span className="text-green-200">4 Available Now</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          ...
          <p className="text-2xl font-bold text-purple-600">{stats.withEmissions}</p>
        </div>
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 text-gray-500 text-sm mb-1">
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          ...
          <p className="text-2xl font-bold text-teal-600">{stats.withTargets}</p>
        </div>
      </div>
