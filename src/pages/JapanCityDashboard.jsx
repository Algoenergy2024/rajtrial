...
function StatCard({ icon: Icon, label, value, subtext, color = 'pink' }) {
  const colors = {
...
function HazardCard({ hazard }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 hover:border-pink-300 transition-colors">
...
function JapanCityDashboard() {
  const { selectedEntity, isJapanCity, selectedId } = useOrganization()
  const [activeTab, setActiveTab] = useState('overview')

  const city = japanCitiesFull.find(c => c.id === selectedId) || selectedEntity

  if (!isJapanCity || !city) {
    return (
      <div className="text-center py-12">
        <Globe className="w-16 h-16 text-gray-300 mx-auto mb-4" />
        <h3 className="text-lg font-medium text-gray-600">Select a Japan city</h3>
        <p className="text-gray-500">Choose "Japan" from the category dropdown</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
            ...
            return (
              ...
              >
                ...
                {tab.id === 'hazards' && hazards.length > 0 && (
                  <span className="ml-1 px-1.5 py-0.5 bg-rose-100 text-rose-600 rounded-full text-xs">
                    {hazards.length}
                  </span>
                )}
                {tab.id === 'targets' && targets.length > 0 && (
                  <span className="ml-1 px-1.5 py-0.5 bg-pink-100 text-pink-600 rounded-full text-xs">
                    {targets.length}
                  </span>
                )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-pink-50 rounded-lg p-4">
                  <p className="text-xs text-pink-600 mb-1">Scope 1 (Direct)</p>
                  <p className="text-xl font-bold text-pink-700">{formatNumber(Math.round(emissions.scope1))}</p>
                </div>
                <div className="bg-rose-50 rounded-lg p-4">
                  <p className="text-xs text-rose-600 mb-1">Scope 2 (Grid)</p>
                  <p className="text-xl font-bold text-rose-700">{formatNumber(Math.round(emissions.scope2))}</p>
                </div>
                <div className="bg-fuchsia-50 rounded-lg p-4">
                  <p className="text-xs text-fuchsia-600 mb-1">Scope 3 (Outside)</p>
                  <p className="text-xl font-bold text-fuchsia-700">{formatNumber(Math.round(emissions.scope3))}</p>
                </div>
      ...
      {/* Data Source Footer */}
      <div className="bg-pink-50 rounded-lg p-4 text-sm text-pink-700">
        <div className="flex items-start space-x-2">
          <Info className="w-4 h-4 mt-0.5 text-pink-600" />
          <div>
            <span className="font-medium">Data Source:</span> CDP 2025 Full Cities Public Data (Japan Region)
            <br />
