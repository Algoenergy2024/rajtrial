...
function ChinaCitiesDashboard() {
  const { setSelectedId, selectedCountry, setSelectedCountry, allChinaCities, chinaCountries } = useOrganization()
  const [sortBy, setSortBy] = useState('name')

  const chinaCities = allChinaCities || []
  const countries = chinaCountries || []

  // Filter by country if selected
  const filteredCities = selectedCountry === 'all'
    ? chinaCities
    : chinaCities.filter(c => c.country === selectedCountry)
  ...
  const stats = {
    ...
    byCountry: countries.map(country => ({
      country,
      count: chinaCities.filter(c => c.country === country).length,
      population: chinaCities.filter(c => c.country === country).reduce((s, c) => s + getPopulation(c), 0)
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* Hero */}
      <div className="bg-gradient-to-br from-red-600 to-rose-700 rounded-2xl p-8 text-white">
        <div className="flex items-center gap-4 mb-4">
          <div className="w-14 h-14 bg-white/20 rounded-xl flex items-center justify-center">
            <Globe className="w-8 h-8" />
          </div>
          <div>
            <h1 className="text-2xl font-bold">China CDP Region</h1>
            <p className="text-white/80">{stats.totalCities} cities across {stats.countries} regions</p>
          </div>
        </div>
        <p className="text-white/70 text-sm flex items-center gap-2">
          <Info className="w-4 h-4" />
          CDP 2025 city climate disclosures - China region (incl. Hong Kong & Taiwan)
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-red-50 rounded-xl p-4">
            <p className="text-sm text-red-600 font-medium mb-1">Scope 1 (Direct)</p>
            <p className="text-2xl font-bold text-red-700">{formatNumber(stats.emissions.scope1)}</p>
          </div>
          <div className="bg-rose-50 rounded-xl p-4">
            <p className="text-sm text-rose-600 font-medium mb-1">Scope 2 (Grid)</p>
            <p className="text-2xl font-bold text-rose-700">{formatNumber(stats.emissions.scope2)}</p>
          </div>
          <div className="bg-pink-50 rounded-xl p-4">
            <p className="text-sm text-pink-600 font-medium mb-1">Scope 3 (Outside)</p>
            <p className="text-2xl font-bold text-pink-700">{formatNumber(stats.emissions.scope3)}</p>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <h2 className="font-semibold text-gray-800 mb-4">Cities by Region</h2>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">
            {selectedCountry === 'all' ? 'All China Region Cities' : `Cities in ${selectedCountry}`}
          </h2>
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-red-700">
          China CDP region includes cities from mainland China, Hong Kong Special Administrative Region, and Taiwan reporting through the CDP program.
        </p>
      </div>
    </div>
  )
...
export default ChinaCitiesDashboard
