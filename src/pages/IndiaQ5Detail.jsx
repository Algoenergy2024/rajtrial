    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          <p className="text-3xl font-bold text-orange-600">{q5_1.length}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center gap-2 text-gray-500 text-sm mb-2">
          ...
          <p className="text-3xl font-bold text-amber-600">{q5_1_1.length}</p>
        </div>
      </div>
            ...
            {q5_1_1.map((goal, idx) => {
              ...
              return (
                <div key={idx} className="bg-gray-50 rounded-lg border border-gray-200 p-4">
                  ...
                  {Object.entries(goal)
                    .filter(([k]) => !['row_order', 'row_name', 'Climate hazard addressed^', 'Sector addressed^', 'Goal description^', 'Adaptation goal^', 'Target year^'].includes(k))
                    .slice(0, 2)
                    .map(([key, value]) => (
