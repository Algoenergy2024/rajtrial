...
function ChinaCityComparison() {
  const [selectedCities, setSelectedCities] = useState([])
  const [showSelector, setShowSelector] = useState(false)
  const [searchTerm, setSearchTerm] = useState('')

  const cities = chinaCitiesFull
  ...
  const colors = [
    { bg: 'bg-red-100', bar: 'bg-red-500', text: 'text-red-700', border: 'border-red-200' },
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-br from-red-600 to-rose-700 rounded-2xl p-8 text-white">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">China City Comparison</h1>
            <p className="text-white/80">Compare emissions, hazards, and climate actions across China region cities</p>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {selectedCities.map((city, idx) => (
              ...
              >
                ...
                <span className={`text-xs ${colors[idx].text}`}>({city.country})</span>
                <button
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-red-50 rounded-lg p-4">
                <p className="text-sm text-red-600 font-medium mb-3">Scope 1 (Direct)</p>
                <div className="space-y-2">
                  {comparisonData.map((data, idx) => (
                    <div key={data.city.id} className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium text-red-700">{formatNumber(data.emissions.scope1)}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-rose-50 rounded-lg p-4">
                <p className="text-sm text-rose-600 font-medium mb-3">Scope 2 (Grid)</p>
                <div className="space-y-2">
                  {comparisonData.map((data, idx) => (
                    <div key={data.city.id} className="flex items-center justify-between">
                      <span className={`text-xs ${colors[idx].text}`}>{data.city.name}</span>
                      <span className="text-sm font-medium text-rose-700">{formatNumber(data.emissions.scope2)}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-pink-50 rounded-lg p-4">
                <p className="text-sm text-pink-600 font-medium mb-3">Scope 3 (Outside)</p>
                <div className="space-y-2">
                  {comparisonData.map((data, idx) => (
                    <div key={data.city.id} className="flex items-center justify-between">
                      <span className={`text-xs ${colors[idx].text}`}>{data.city.name}</span>
                      <span className="text-sm font-medium text-pink-700">{formatNumber(data.emissions.scope3)}</span>
                    </div>
                  ))}
                </div>
              </div>
...
export default ChinaCityComparison
