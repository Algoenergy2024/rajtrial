...
function JapanCitiesDashboard() {
  const { setSelectedId, selectedCountry, setSelectedCountry, allJapanCities, japanCountries } = useOrganization()
  const [sortBy, setSortBy] = useState('name')

  const japanCities = allJapanCities || []
  const countries = japanCountries || []

  // Filter by country if selected
  const filteredCities = selectedCountry === 'all'
    ? japanCities
    : japanCities.filter(c => c.country === selectedCountry)
  ...
  // Compute aggregates
  const stats = {
    totalCities: japanCities.length,
    ...
    byCountry: countries.map(country => ({
      country,
      count: japanCities.filter(c => c.country === country).length,
      population: japanCities.filter(c => c.country === country).reduce((s, c) => s + getPopulation(c), 0)
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-pink-50 rounded-xl p-4">
            <p className="text-sm text-pink-600 font-medium mb-1">Scope 1 (Direct)</p>
            <p className="text-2xl font-bold text-pink-700">{formatNumber(stats.emissions.scope1)}</p>
          </div>
          ...
          <div className="bg-fuchsia-50 rounded-xl p-4">
            <p className="text-sm text-fuchsia-600 font-medium mb-1">Scope 3 (Outside)</p>
            <p className="text-2xl font-bold text-fuchsia-700">{formatNumber(stats.emissions.scope3)}</p>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">
            {selectedCountry === 'all' ? 'All Japan Region Cities' : `Cities in ${selectedCountry}`}
          </h2>
        ...
        <div className="divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
          {sortedCities.map(city => (
            <button
              ...
              className="w-full px-6 py-4 flex items-center justify-between hover:bg-pink-50 transition-colors text-left"
            >
      ...
      <div className="bg-pink-50 border border-pink-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-pink-700">
          Japan CDP region includes cities from Japan reporting through the CDP program.
        </p>
      </div>
    </div>
  )
