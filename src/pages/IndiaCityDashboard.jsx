...
function IndiaCityDashboard() {
  const { selectedEntity, isIndiaCity, selectedId } = useOrganization()
  const [activeTab, setActiveTab] = useState('overview')

  const city = indiaCitiesFull.find(c => c.id === selectedId) || selectedEntity

  if (!isIndiaCity || !city) {
    return (
      <div className="text-center py-12">
        <Globe className="w-16 h-16 text-gray-300 mx-auto mb-4" />
        <h3 className="text-lg font-medium text-gray-600">Select an India city</h3>
        <p className="text-gray-500">Choose "India" from the category dropdown</p>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-orange-50 rounded-lg p-4">
                  <p className="text-xs text-orange-600 mb-1">Scope 1 (Direct)</p>
                  <p className="text-xl font-bold text-orange-700">{formatNumber(Math.round(emissions.scope1))}</p>
                </div>
                <div className="bg-amber-50 rounded-lg p-4">
                  <p className="text-xs text-amber-600 mb-1">Scope 2 (Grid)</p>
                  <p className="text-xl font-bold text-amber-700">{formatNumber(Math.round(emissions.scope2))}</p>
                </div>
      ...
      <div className="bg-orange-50 rounded-lg p-4 text-sm text-orange-700">
        <div className="flex items-start space-x-2">
          ...
          <div>
            <span className="font-medium">Data Source:</span> CDP 2025 Full Cities Public Data (India Region)
            <br />
