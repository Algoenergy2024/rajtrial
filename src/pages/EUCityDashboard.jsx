    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4 hover:border-orange-300 transition-colors">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h4 className="font-semibold text-gray-800">{hazard.hazard || hazard['Hazard^'] || 'Unknown Hazard'}</h4>
            <p className="text-xs text-gray-500">{hazard.timeframe || hazard['Timeframe^'] || ''}</p>
          </div>
        </div>
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${probColors[hazard.currentProbability || hazard['Current probability of hazard^']] || 'bg-gray-100 text-gray-600'}`}>
          {hazard.currentProbability || hazard['Current probability of hazard^'] || 'Unknown'}
        </span>
      </div>
      {(hazard.impacts || hazard['Impact(s) experienced^']) && (
        <p className="text-sm text-gray-600 line-clamp-3">{hazard.impacts || hazard['Impact(s) experienced^']}</p>
      )}
    </div>
  )
}
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4 hover:border-green-300 transition-colors">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h4 className="font-semibold text-gray-800">{target['Sector^'] || target.sector || 'City-wide'}</h4>
            <p className="text-xs text-gray-500">{target['Target year^'] || target.year || ''}</p>
          </div>
        </div>
      </div>
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span className="font-medium text-green-600">{target['Percentage reduction target^'] || target.reduction || 'N/A'}%</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-500">Baseline Year</span>
          <span className="font-medium text-gray-700">{target['Baseline year^'] || target.baseline || 'N/A'}</span>
        </div>
      </div>
    </div>
  )
      ...
      <div className="flex items-start space-x-3 mb-3">
        ...
        <div>
          <h4 className="font-semibold text-gray-800 line-clamp-2">
            {action['Action^'] || action['Mitigation action^'] || action.action || 'Climate Action'}
          </h4>
          <p className="text-xs text-gray-500">{action['Primary emissions sector^'] || action.sector || ''}</p>
        </div>
      </div>
...
function EUCityDashboard() {
  ...
  const profile = city.q1_2?.[0] || {}
  const population = profile.population || profile['Population^'] || 0
  const area = profile['Land area (in square km)^'] || profile.area || 0
                  ...
                  <tbody>
                    {city.q3_1_3.map((sector, i) => (
                      <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="py-3 px-4 text-gray-800">{sector['Sector^'] || 'Unknown'}</td>
            ...
            <div className="space-y-4">
              {plans.map((plan, i) => (
                <div key={i} className="bg-gray-50 rounded-lg p-4">
                  <h4 className="font-medium text-gray-800 mb-2">{plan['Type of climate action plan^'] || 'Climate Plan'}</h4>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      <span className="text-gray-500">Status:</span>
                      <span className="ml-2 text-gray-700">{plan['Stage of implementation^'] || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Year Adopted:</span>
                      <span className="ml-2 text-gray-700">{plan['Year of adoption^'] || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Implementation Year:</span>
                      <span className="ml-2 text-gray-700">{plan['Year of implementation^'] || 'N/A'}</span>
                    </div>
                  </div>
                </div>
              ))}
