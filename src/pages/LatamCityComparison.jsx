...
function LatamCityComparison() {
  const { isLatamCity, latamCountries, selectedCountry, setSelectedCountry } = useOrganization()
  ...
  if (!isLatamCity) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-600">Select Latin America Cities category</h3>
        <p className="text-gray-500">Switch to Latin America Cities to view comparison</p>
      </div>
    )
  }
  ...
  const filteredCities = selectedCountry === 'all'
    ? latamCitiesFull
    : latamCitiesFull.filter(c => c.country === selectedCountry)
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-2xl p-6 text-white">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">Latin America City Comparison</h1>
            <p className="text-white/80">Compare CDP reporting metrics across Latin America cities</p>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {sortedCities.map((city, idx) => (
                <tr key={city.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`px-2 py-1 rounded text-sm ${
                      city.mitigationActions > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'
                    }`}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`px-2 py-1 rounded text-sm ${
                      city.plans > 0 ? 'bg-teal-100 text-teal-700' : 'bg-gray-100 text-gray-500'
                    }`}>
                      {city.plans}
                    </span>
                  </td>
...
export default LatamCityComparison
