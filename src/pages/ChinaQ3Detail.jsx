      ...
      {q3_1_3.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-red-50 rounded-xl p-5 border border-red-100">
            ...
            <p className="text-2xl font-bold text-red-700">{formatNumber(Math.round(scope1Total))}</p>
            <p className="text-xs text-red-500">tCO2e</p>
          </div>
          <div className="bg-rose-50 rounded-xl p-5 border border-rose-100">
            <p className="text-sm text-rose-600 font-medium mb-1">Scope 2 (Grid)</p>
            <p className="text-2xl font-bold text-rose-700">{formatNumber(Math.round(scope2Total))}</p>
            <p className="text-xs text-rose-500">tCO2e</p>
          </div>
          <div className="bg-pink-50 rounded-xl p-5 border border-pink-100">
            ...
            <p className="text-2xl font-bold text-pink-700">{formatNumber(Math.round(scope3Total))}</p>
            <p className="text-xs text-pink-500">tCO2e</p>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {q3_1_3.map((sector, idx) => {
                  ...
                  return (
                    <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-3 text-right text-red-600">{formatNumber(Math.round(s1))}</td>
                      <td className="py-3 px-3 text-right text-rose-600">{formatNumber(Math.round(s2))}</td>
                      <td className="py-3 px-3 text-right text-pink-600">{formatNumber(Math.round(s3))}</td>
                ...
                <tr className="bg-gray-50 font-semibold">
                  ...
                  <td className="py-3 px-3 text-right text-red-700">{formatNumber(Math.round(scope1Total))}</td>
                  <td className="py-3 px-3 text-right text-rose-700">{formatNumber(Math.round(scope2Total))}</td>
                  <td className="py-3 px-3 text-right text-pink-700">{formatNumber(Math.round(scope3Total))}</td>
