...
import {
  Globe, BookOpen, FileText, Target, Calendar, Info
} from 'lucide-react'
import { useOrganization } from '../context/OrganizationContext'
import { northAmericaCitiesFull } from '../data/northAmericaCitiesFull'

function NorthAmericaQ8Detail() {
  ...
  if (!hasData) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500">This city has not submitted Q8 Strategy data</p>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-sky-500 to-blue-600 rounded-2xl p-6 text-white">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-2xl font-bold">Q8 - Climate Action Plans & Strategy</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          <p className="text-3xl font-bold text-sky-600">{q8_1.length}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center gap-2 text-gray-500 text-sm mb-2">
          ...
          <p className="text-3xl font-bold text-blue-600">{q8_1_1.length}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center gap-2 text-gray-500 text-sm mb-2">
      ...
      {q8_1_1.length > 0 && (
        <div id="q8_1_1" className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden scroll-mt-20">
          ...
          <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            {q8_1_1.map((plan, idx) => {
              const planName = plan['Plan name^'] || plan['Title of plan^'] || plan.row_name || 'Climate Plan'
              const year = plan['Year of adoption^'] || plan['Year of publication^'] || ''
              const stage = plan['Stage of plan^'] || ''
              ...
              return (
                <div key={idx} className="bg-gray-50 rounded-lg border border-gray-200 p-4">
                  ...
                  {Object.entries(plan)
                    .filter(([k]) => !['row_order', 'row_name', 'question_text', 'Plan name^', 'Title of plan^', 'Year of adoption^', 'Year of publication^', 'Stage of plan^'].includes(k))
                    .slice(0, 2)
                    .map(([key, value]) => (
                      <p key={key} className="text-sm text-gray-600 mt-2">
...
export default NorthAmericaQ8Detail
