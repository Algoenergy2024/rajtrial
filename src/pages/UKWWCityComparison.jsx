...
function UKWWCityComparison() {
  const [selectedCities, setSelectedCities] = useState([])
  const [showSelector, setShowSelector] = useState(false)
  const [searchTerm, setSearchTerm] = useState('')

  const cities = euCitiesFull.filter(c => UKWW_COUNTRIES.includes(c.country))
  ...
  const colors = [
    ...
    { bg: 'bg-orange-100', bar: 'bg-orange-500', text: 'text-orange-700', border: 'border-orange-200' },
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-br from-amber-600 to-orange-700 rounded-2xl p-8 text-white">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">UKWW City Comparison</h1>
            <p className="text-white/80">Compare emissions, hazards, and climate actions across UKWW cities</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-amber-50 rounded-lg p-4">
                <p className="text-sm text-amber-600 font-medium mb-3">Scope 1 (Direct)</p>
                <div className="space-y-2">
                  {comparisonData.map((data, idx) => (
                    <div key={data.city.id} className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium text-amber-700">{formatNumber(data.emissions.scope1)}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-orange-50 rounded-lg p-4">
                <p className="text-sm text-orange-600 font-medium mb-3">Scope 2 (Grid)</p>
                <div className="space-y-2">
                  {comparisonData.map((data, idx) => (
                    <div key={data.city.id} className="flex items-center justify-between">
                      <span className={`text-xs ${colors[idx].text}`}>{data.city.name}</span>
                      <span className="text-sm font-medium text-orange-700">{formatNumber(data.emissions.scope2)}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-yellow-50 rounded-lg p-4">
                <p className="text-sm text-yellow-600 font-medium mb-3">Scope 3 (Outside)</p>
                <div className="space-y-2">
                  {comparisonData.map((data, idx) => (
                    <div key={data.city.id} className="flex items-center justify-between">
                      <span className={`text-xs ${colors[idx].text}`}>{data.city.name}</span>
                      <span className="text-sm font-medium text-yellow-700">{formatNumber(data.emissions.scope3)}</span>
                    </div>
                  ))}
                </div>
              </div>
...
export default UKWWCityComparison
