...
function StatCard({ icon: Icon, label, value, subtext, color = 'red' }) {
  const colors = {
...
function ChinaCityDashboard() {
  const { selectedEntity, isChinaCity, selectedId } = useOrganization()
  const [activeTab, setActiveTab] = useState('overview')

  const city = chinaCitiesFull.find(c => c.id === selectedId) || selectedEntity

  if (!isChinaCity || !city) {
    return (
      <div className="text-center py-12">
        <Globe className="w-16 h-16 text-gray-300 mx-auto mb-4" />
        <h3 className="text-lg font-medium text-gray-600">Select a China city</h3>
        <p className="text-gray-500">Choose "China" from the category dropdown</p>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-red-50 rounded-lg p-4">
                  <p className="text-xs text-red-600 mb-1">Scope 1 (Direct)</p>
                  <p className="text-xl font-bold text-red-700">{formatNumber(Math.round(emissions.scope1))}</p>
                </div>
                ...
                <div className="bg-pink-50 rounded-lg p-4">
                  <p className="text-xs text-pink-600 mb-1">Scope 3 (Outside)</p>
                  <p className="text-xl font-bold text-pink-700">{formatNumber(Math.round(emissions.scope3))}</p>
                </div>
      ...
      <div className="bg-red-50 rounded-lg p-4 text-sm text-red-700">
        <div className="flex items-start space-x-2">
          ...
          <div>
            <span className="font-medium">Data Source:</span> CDP 2025 Full Cities Public Data (China Region)
            <br />
...
export default ChinaCityDashboard
