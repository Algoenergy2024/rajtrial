...
function StatCard({ icon: Icon, label, value, subtext, color = 'emerald' }) {
  const colors = {
...
function LatamCityDashboard() {
  const { selectedEntity, isLatamCity, selectedId } = useOrganization()
  const [activeTab, setActiveTab] = useState('overview')

  const city = latamCitiesFull.find(c => c.id === selectedId) || selectedEntity

  if (!isLatamCity || !city) {
    return (
      <div className="text-center py-12">
        <Globe className="w-16 h-16 text-gray-300 mx-auto mb-4" />
        <h3 className="text-lg font-medium text-gray-600">Select a Latin America city</h3>
        <p className="text-gray-500">Choose "Latin America" from the category dropdown</p>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
            ...
            return (
              ...
              >
                ...
                {tab.id === 'targets' && targets.length > 0 && (
                  <span className="ml-1 px-1.5 py-0.5 bg-emerald-100 text-emerald-600 rounded-full text-xs">
                    {targets.length}
                  </span>
                )}
                {tab.id === 'actions' && actions.length > 0 && (
                  <span className="ml-1 px-1.5 py-0.5 bg-cyan-100 text-cyan-600 rounded-full text-xs">
                    {actions.length}
                  </span>
                )}
              </button>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-emerald-50 rounded-lg p-4">
                  <p className="text-xs text-emerald-600 mb-1">Scope 1 (Direct)</p>
                  <p className="text-xl font-bold text-emerald-700">{formatNumber(Math.round(emissions.scope1))}</p>
                </div>
                <div className="bg-teal-50 rounded-lg p-4">
                  <p className="text-xs text-teal-600 mb-1">Scope 2 (Grid)</p>
                  <p className="text-xl font-bold text-teal-700">{formatNumber(Math.round(emissions.scope2))}</p>
                </div>
                <div className="bg-cyan-50 rounded-lg p-4">
                  <p className="text-xs text-cyan-600 mb-1">Scope 3 (Outside)</p>
                  <p className="text-xl font-bold text-cyan-700">{formatNumber(Math.round(emissions.scope3))}</p>
                </div>
      ...
      <div className="bg-emerald-50 rounded-lg p-4 text-sm text-emerald-700">
        <div className="flex items-start space-x-2">
          ...
          <div>
            <span className="font-medium">Data Source:</span> CDP 2025 Full Cities Public Data (Latin America Region)
            <br />
...
export default LatamCityDashboard
