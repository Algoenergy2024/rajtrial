...
function NorthAmericaCitiesDashboard() {
  const { setSelectedId, selectedCountry, setSelectedCountry, allNorthAmericaCities, northAmericaCountries } = useOrganization()
  const [sortBy, setSortBy] = useState('name')

  const naCities = allNorthAmericaCities || []
  const countries = northAmericaCountries || []

  const filteredCities = selectedCountry === 'all'
    ? naCities
    : naCities.filter(c => c.country === selectedCountry)
  ...
  const stats = {
    ...
    byCountry: countries.map(country => ({
      country,
      count: naCities.filter(c => c.country === country).length,
      population: naCities.filter(c => c.country === country).reduce((s, c) => s + getPopulation(c), 0)
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-br from-sky-500 to-blue-600 rounded-2xl p-8 text-white">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">North America CDP Region</h1>
            <p className="text-white/80">{stats.totalCities} cities across {stats.countries} countries</p>
          </div>
        </div>
        <p className="text-white/70 text-sm flex items-center gap-2">
          <Info className="w-4 h-4" />
          CDP 2025 city climate disclosures - North America region
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-sky-50 rounded-xl p-4">
            <p className="text-sm text-sky-600 font-medium mb-1">Scope 1 (Direct)</p>
            <p className="text-2xl font-bold text-sky-700">{formatNumber(stats.emissions.scope1)}</p>
          </div>
          <div className="bg-blue-50 rounded-xl p-4">
            <p className="text-sm text-blue-600 font-medium mb-1">Scope 2 (Grid)</p>
            <p className="text-2xl font-bold text-blue-700">{formatNumber(stats.emissions.scope2)}</p>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">
            {selectedCountry === 'all' ? 'All North America Cities' : `Cities in ${selectedCountry}`}
          </h2>
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="text-sm border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-sky-500"
            >
        ...
        <div className="divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
          {sortedCities.map(city => (
            <button
              ...
              className="w-full px-6 py-4 flex items-center justify-between hover:bg-sky-50 transition-colors text-left"
            >
      ...
      <div className="bg-sky-50 border border-sky-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-sky-700">
          North America CDP region includes cities from the United States of America and Canada reporting through the CDP program.
        </p>
      </div>
    </div>
  )
...
export default NorthAmericaCitiesDashboard
