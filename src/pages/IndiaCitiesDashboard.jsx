...
function IndiaCitiesDashboard() {
  const { setSelectedId, selectedCountry, setSelectedCountry, allIndiaCities, indiaCountries } = useOrganization()
  const [sortBy, setSortBy] = useState('name')

  const indiaCities = allIndiaCities || []
  const countries = indiaCountries || []

  // Filter by country if selected
  const filteredCities = selectedCountry === 'all'
    ? indiaCities
    : indiaCities.filter(c => c.country === selectedCountry)
  ...
  // Compute aggregates
  const stats = {
    totalCities: indiaCities.length,
    ...
    byCountry: countries.map(country => ({
      country,
      count: indiaCities.filter(c => c.country === country).length,
      population: indiaCities.filter(c => c.country === country).reduce((s, c) => s + getPopulation(c), 0)
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-orange-50 rounded-xl p-4">
            <p className="text-sm text-orange-600 font-medium mb-1">Scope 1 (Direct)</p>
            <p className="text-2xl font-bold text-orange-700">{formatNumber(stats.emissions.scope1)}</p>
          </div>
          <div className="bg-amber-50 rounded-xl p-4">
            <p className="text-sm text-amber-600 font-medium mb-1">Scope 2 (Grid)</p>
            <p className="text-2xl font-bold text-amber-700">{formatNumber(stats.emissions.scope2)}</p>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">
            {selectedCountry === 'all' ? 'All India Region Cities' : `Cities in ${selectedCountry}`}
          </h2>
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-orange-700">
          India CDP region includes cities from India reporting through the CDP program.
        </p>
      </div>
    </div>
  )
...
export default IndiaCitiesDashboard
