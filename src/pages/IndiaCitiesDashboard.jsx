...
function IndiaCitiesDashboard() {
  const { setSelectedId, indiaCountries, selectedCountry, setSelectedCountry } = useOrganization()
  const navigate = useNavigate()

  const filteredCities = selectedCountry === 'all'
    ? indiaCitiesFull
    : indiaCitiesFull.filter(c => c.country === selectedCountry)
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-amber-600 rounded-2xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-3xl font-bold">India Cities Dashboard</h1>
              <p className="text-white/80">CDP 2025 Full Cities Public Data - India Region</p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <div className="flex items-center gap-4">
          ...
          >
            <option value="all">All Countries ({indiaCitiesFull.length})</option>
            {indiaCountries.map(country => (
              <option key={country} value={country}>
                {country} ({indiaCitiesFull.filter(c => c.country === country).length})
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          <p className="text-3xl font-bold text-orange-600">{filteredCities.length}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center gap-2 text-gray-500 text-sm mb-2">
          ...
          <p className="text-3xl font-bold text-amber-600">{(totalPopulation / 1000000).toFixed(1)}M</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center gap-2 text-gray-500 text-sm mb-2">
          ...
          {filteredCities.map((city) => {
            ...
            return (
              <button
                ...
                className="text-left bg-gray-50 hover:bg-orange-50 rounded-lg border border-gray-200 hover:border-orange-300 p-4 transition-all"
              >
