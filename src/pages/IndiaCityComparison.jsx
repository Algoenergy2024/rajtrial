...
function IndiaCityComparison() {
  const { isIndiaCity, indiaCountries, selectedCountry, setSelectedCountry } = useOrganization()
  ...
  if (!isIndiaCity) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-600">Select India Cities category</h3>
        <p className="text-gray-500">Switch to India Cities to view comparison</p>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-amber-600 rounded-2xl p-6 text-white">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">India City Comparison</h1>
            <p className="text-white/80">Compare CDP reporting metrics across India cities</p>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {sortedCities.map((city, idx) => (
                <tr key={city.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`px-2 py-1 rounded text-sm ${
                      city.mitigationActions > 0 ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'
                    }`}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`px-2 py-1 rounded text-sm ${
                      city.plans > 0 ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-500'
                    }`}>
                      {city.plans}
                    </span>
                  </td>
...
export default IndiaCityComparison
